...
/opt/benchcpu/llvm-profiling/dynamic-instruction-count/cdi $log_dir/$exe_name.ll $log_dir/$exe_name.bc > $log_dir/static_insmix.log \
    || (echo "instrumentation failed"; exit 1)
llc -filetype=obj $log_dir/$exe_name.bc -o $log_dir/$exe_name.o \
    || (echo "llc failed"; exit 1)
clang++ $log_dir/$exe_name.o /opt/benchcpu/llvm-profiling/runtime.cpp \
